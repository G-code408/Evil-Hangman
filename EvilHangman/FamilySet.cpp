...
void FamilySet::filterFamilies(string letter, string guessPattern)
{
	...
	for(auto it = masterList.begin(); it!= masterList.end(); it++)
    {
        ...
        {
            string currLetter = currLetter + word[i];
            string familyName = guessPattern;
...
void FamilySet::setFamily(string &family)
{
    ...
    masterList.clear();
    //cout << "MASTER SIZE: " << masterList.size() << endl;
	int number = 0;
	string largestFam;
    ...
    for(int i=0; i<dictionaries[largestFam].size(); i++)
    {
        ...
        //cout << "adding " << dictionaries[largestFam].at(i) << "  " << dictionaries[largestFam].at(i) << endl;
    }
    family = largestFam;
    dictionaries.clear();
}
